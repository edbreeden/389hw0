...
for i in range(10):
    ...
    t = timeit.timeit(stmt = "update_coords(x, y, z, vx, vy, vz)",
                      ...
                      number = iters)
    s = 1000000 * t / (size * iters)
    ss.append(s)
   # print("Mean time per coordinate: " + str(s) + "\n")
avg = sum(ss)/10

devs = []
for itm in ss:
    devs.append(abs(avg-itm))
#print(str(abs(avg-itm)))
#print(str(devs))
stdv = sum(devs)/10
print("STD DEV is:" + str(stdv) + "\n")
"""    
chksum = sum(x) + sum(y) + sum(z)
print("Mean time per coordinate: " + str(1000000 * t / (size * iters)) + "us")
print("Final checksum is: " + str(chksum))
print("Total time: " + str(100 * t) + "\n")
"""
exit(0)
